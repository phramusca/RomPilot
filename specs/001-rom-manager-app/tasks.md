...
**Goal**: Scanner récursivement des répertoires pour trouver des ROMs (y compris dans archives ZIP/7Z), calculer checksums, identifier console et jeu via bases de données

**Independent Test**: Fournir un répertoire de test avec ROMs dans différents formats (fichiers directs, ZIP, 7Z, archives imbriquées). Vérifier que tous les ROMs sont détectés, consoles identifiées, jeux reconnus via bases de données, et checksums calculés.

### Tests for User Story 1 (OPTIONAL - TDD approach) ⚠️

> **NOTE: Write these tests FIRST, ensure they FAIL before implementation**

- [ ] T045 [P] [US1] Unit test for ArchiveScanner in src/RomPilot.Tests/Unit/Archives/ArchiveScannerTests.cs
- [ ] T046 [P] [US1] Unit test for ChecksumCalculator in src/RomPilot.Tests/Unit/Checksums/ChecksumCalculatorTests.cs
- [ ] T047 [P] [US1] Unit test for ConsoleDetectionService in src/RomPilot.Tests/Unit/Services/ConsoleDetectionServiceTests.cs
- [ ] T048 [P] [US1] Unit test for ScanService in src/RomPilot.Tests/Unit/Services/ScanServiceTests.cs
- [ ] T049 [US1] Integration test for scan workflow in src/RomPilot.Tests/Integration/ScanServiceIntegrationTests.cs
...
- [ ] T070 [P] [US2] Unit test for GameGroupingService in src/RomPilot.Tests/Unit/Services/GameGroupingServiceTests.cs
- [ ] T071 [P] [US2] Unit test for VersionSelectionService in src/RomPilot.Tests/Unit/Services/VersionSelectionServiceTests.cs
- [ ] T072 [US2] Integration test for filtering workflow in src/RomPilot.Tests/Integration/FilteringIntegrationTests.cs

### Implementation for User Story 2

- [ ] T073 [US2] Create game grouping service interface IGameGroupingService in src/RomPilot.Core/Services/IGameGroupingService.cs
- [ ] T074 [US2] Create game grouping service implementation GameGroupingService in src/RomPilot.Core/Services/GameGroupingService.cs (groups ROMs by game)
- [ ] T075 [US2] Create version selection service interface IVersionSelectionService in src/RomPilot.Core/Services/IVersionSelectionService.cs
- [ ] T076 [US2] Create version selection service implementation VersionSelectionService in src/RomPilot.Core/Services/VersionSelectionService.cs (selects best version based on preferences)
- [ ] T077 [US2] Create preference configuration view model PreferencesViewModel in src/RomPilot.UI/ViewModels/PreferencesViewModel.cs
- [ ] T078 [US2] Create preference configuration view PreferencesView in src/RomPilot.UI/Views/PreferencesView.axaml (region priority, language priority, exclude bad dumps)
- [ ] T079 [US2] Create game list view model GameListViewModel in src/RomPilot.UI/ViewModels/GameListViewModel.cs (displays grouped games with selected versions)
- [ ] T080 [US2] Create game list view GameListView in src/RomPilot.UI/Views/GameListView.axaml (list of games with version info)
- [ ] T081 [US2] Integrate grouping and filtering with UI in src/RomPilot.UI/ViewModels/GameListViewModel.cs (apply preferences, update selection)
- [ ] T082 [US2] Add manual version override capability in src/RomPilot.UI/ViewModels/GameListViewModel.cs (user can manually select different version)
...
- [ ] T083 [P] [US3] Unit test for RecalboxExportAdapter in src/RomPilot.Tests/Unit/PlatformAdapters/RecalboxExportAdapterTests.cs
- [ ] T084 [P] [US3] Unit test for RommExportAdapter in src/RomPilot.Tests/Unit/PlatformAdapters/RommExportAdapterTests.cs
- [ ] T085 [US3] Integration test for export workflow in src/RomPilot.Tests/Integration/ExportIntegrationTests.cs

### Implementation for User Story 3

- [ ] T086 [US3] Create export service interface IExportService in src/RomPilot.Core/Services/IExportService.cs
- [ ] T087 [US3] Create export service implementation ExportService in src/RomPilot.Core/Services/ExportService.cs (orchestrates export)
- [ ] T088 [P] [US3] Create platform adapter interface IPlatformAdapter in src/RomPilot.Core/PlatformAdapters/IPlatformAdapter.cs
- [ ] T089 [US3] Create Recalbox export adapter RecalboxExportAdapter in src/RomPilot.Core/PlatformAdapters/RecalboxExportAdapter.cs (folder naming, format conversion)
- [ ] T090 [US3] Create Romm export adapter RommExportAdapter in src/RomPilot.Core/PlatformAdapters/RommExportAdapter.cs (folder structure, API integration)
- [ ] T091 [US3] Create console folder name mapping service ConsoleFolderMappingService in src/RomPilot.Core/Services/ConsoleFolderMappingService.cs (Recalbox and Romm conventions)
- [ ] T092 [US3] Create export configuration view model ExportViewModel in src/RomPilot.UI/ViewModels/ExportViewModel.cs
- [ ] T093 [US3] Create export configuration view ExportView in src/RomPilot.UI/Views/ExportView.axaml (platform selection, destination path, options)
- [ ] T094 [US3] Create export progress view model ExportProgressViewModel in src/RomPilot.UI/ViewModels/ExportProgressViewModel.cs
- [ ] T095 [US3] Create export progress view ExportProgressView in src/RomPilot.UI/Views/ExportProgressView.axaml (progress bar, file list)
- [ ] T096 [US3] Integrate export service with UI in src/RomPilot.UI/ViewModels/ExportViewModel.cs (async export, progress updates)
- [ ] T097 [US3] Add conflict resolution handling in src/RomPilot.Core/Services/ExportService.cs (file exists, user confirmation)
- [ ] T098 [US3] Create export history repository interface IExportHistoryRepository in src/RomPilot.Core/Repositories/IExportHistoryRepository.cs
- [ ] T099 [US3] Create export history repository implementation ExportHistoryRepository in src/RomPilot.Core/Repositories/ExportHistoryRepository.cs
...
- [ ] T100 [P] [US4] Unit test for RecalboxMetadataAdapter in src/RomPilot.Tests/Unit/PlatformAdapters/RecalboxMetadataAdapterTests.cs
- [ ] T101 [P] [US4] Unit test for RommApiClient in src/RomPilot.Tests/Unit/PlatformAdapters/RommApiClientTests.cs
- [ ] T102 [US4] Integration test for sync workflow in src/RomPilot.Tests/Integration/SyncIntegrationTests.cs

### Implementation for User Story 4

- [ ] T103 [US4] Create sync service interface ISyncService in src/RomPilot.Core/Services/ISyncService.cs
- [ ] T104 [US4] Create sync service implementation SyncService in src/RomPilot.Core/Services/SyncService.cs (orchestrates sync)
- [ ] T105 [US4] Create Recalbox metadata adapter RecalboxMetadataAdapter in src/RomPilot.Core/PlatformAdapters/RecalboxMetadataAdapter.cs (read/write gamelist.xml)
- [ ] T106 [US4] Create Romm API client interface IRommApiClient in src/RomPilot.Core/PlatformAdapters/IRommApiClient.cs
- [ ] T107 [US4] Create Romm API client implementation RommApiClient in src/RomPilot.Core/PlatformAdapters/RommApiClient.cs (HTTP client, endpoints)
- [ ] T108 [US4] Create metadata mapping service MetadataMappingService in src/RomPilot.Core/Services/MetadataMappingService.cs (maps between formats)
- [ ] T109 [US4] Create conflict resolution service ConflictResolutionService in src/RomPilot.Core/Services/ConflictResolutionService.cs (last modified wins, user confirmation)
- [ ] T110 [US4] Create sync configuration view model SyncViewModel in src/RomPilot.UI/ViewModels/SyncViewModel.cs
- [ ] T111 [US4] Create sync configuration view SyncView in src/RomPilot.UI/Views/SyncView.axaml (platform selection, sync direction, options)
- [ ] T112 [US4] Create sync progress view model SyncProgressViewModel in src/RomPilot.UI/ViewModels/SyncProgressViewModel.cs
- [ ] T113 [US4] Create sync progress view SyncProgressView in src/RomPilot.UI/Views/SyncProgressView.axaml (progress bar, conflicts list)
- [ ] T114 [US4] Integrate sync service with UI in src/RomPilot.UI/ViewModels/SyncViewModel.cs (async sync, progress updates)
- [ ] T115 [US4] Add conflict resolution UI ConflictResolutionView in src/RomPilot.UI/Views/ConflictResolutionView.axaml (show conflicts, user choice)
- [ ] T116 [US4] Create sync history repository interface ISyncHistoryRepository in src/RomPilot.Core/Repositories/ISyncHistoryRepository.cs
- [ ] T117 [US4] Create sync history repository implementation SyncHistoryRepository in src/RomPilot.Core/Repositories/SyncHistoryRepository.cs
...
- [ ] T118 [P] Documentation updates in README.md (installation, usage, features)
- [ ] T119 [P] Code cleanup and refactoring in src/ (remove unused code, improve naming)
- [ ] T120 [P] Performance optimization across all stories in src/RomPilot.Core/ (profiling, bottlenecks)
- [ ] T121 [P] Additional unit tests in src/RomPilot.Tests/Unit/ (increase coverage to 80% critical, 60% rest)
- [ ] T122 [P] Security hardening in src/RomPilot.Core/ (validate user inputs, sanitize paths, secure API key storage)
- [ ] T123 [P] Error handling improvements in src/RomPilot.Core/ (comprehensive error messages, logging)
- [ ] T124 [P] UI/UX polish in src/RomPilot.UI/ (icons, tooltips, keyboard shortcuts, accessibility)
- [ ] T125 [P] Localization support preparation in src/RomPilot.UI/ (if needed, prepare for translations)
- [ ] T126 [P] Run quickstart.md validation (verify all setup steps work)
- [ ] T127 [P] Create deployment packages (Linux AppImage/.deb, Windows MSI, macOS .dmg)
...
Task: "Unit test for ArchiveScanner in src/RomPilot.Tests/Unit/Archives/ArchiveScannerTests.cs"
Task: "Unit test for ChecksumCalculator in src/RomPilot.Tests/Unit/Checksums/ChecksumCalculatorTests.cs"
Task: "Unit test for ConsoleDetectionService in src/RomPilot.Tests/Unit/Services/ConsoleDetectionServiceTests.cs"
...
- Total tasks: 127 (Setup: 9, Foundational: 35, US1: 25, US2: 13, US3: 17, US4: 18, Polish: 10)
